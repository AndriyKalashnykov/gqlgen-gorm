...
func (r *mutationResolver) DeleteTodo(ctx context.Context, todoID int) (*customTypes.Todo, error) {
	cc := common.GetContext(ctx)
	// Load the row first so we can return it for the non-null Todo! field;
	// a bare Delete leaves the result nil and fails the schema contract.
	var todo customTypes.Todo
	if err := cc.Database.Where("id = ?", todoID).First(&todo).Error; err != nil {
		return nil, err
	}
	if err := cc.Database.Delete(&todo).Error; err != nil {
		return nil, err
	}
	return &todo, nil
}
